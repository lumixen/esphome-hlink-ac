...
TEST(CircularRequestsQueueTest, EnqueueAndDequeueSingle) {
  CircularRequestsQueue queue;
  auto req = make_unique<HlinkRequest>(HlinkRequestFrame{HlinkRequestFrame::Type::MT, {0x0000}});
...
TEST(CircularRequestsQueueTest, FifoOrder) {
  CircularRequestsQueue queue;
  for (int i = 0; i < 5; i++) {
    auto req = make_unique<HlinkRequest>(HlinkRequestFrame{HlinkRequestFrame::Type::ST, {static_cast<uint16_t>(i)}});
...
TEST(CircularRequestsQueueTest, WrapAround) {
  ...
  for (int i = 0; i < REQUESTS_QUEUE_SIZE / 2; i++) {
    auto r = make_unique<HlinkRequest>(HlinkRequestFrame{HlinkRequestFrame::Type::MT, {0x0000}});
...
TEST(CircularRequestsQueueTest, SizeTracking) {
  ...
  auto req1 = make_unique<HlinkRequest>(HlinkRequestFrame{HlinkRequestFrame::Type::MT, {0x0000}});
  queue.enqueue(std::move(req1));
  EXPECT_EQ(queue.size(), 1);

  auto req2 = make_unique<HlinkRequest>(HlinkRequestFrame{HlinkRequestFrame::Type::MT, {0x0001}});
